...
/*   Updated: 2022/09/23 17:27:49 by jocardos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	init_vars(game);
	count_elements_in_map(game);
	if (!verify_rectangle(game))
		return (0);
	if (check_invalid_char(game))
		return (0);
	if (!rule_num_of_elements(game))
		return (0);
	if (!walls_around_map(game))
		return (0);
	if (!checking_valid_path(game, game->player_x, game->player_y))
		return (0);
	return (1);
}
